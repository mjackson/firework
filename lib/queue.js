 ...
 */
Queue.prototype.removeJob = function (job, callback) {
  if (job._name) {
    this.pendingJobs.child(job._name).remove(callback);
  } else {
    callback(new Error('Cannot remove job without a name'));
  }
};
